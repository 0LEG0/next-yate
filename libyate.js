...
function getEngine(options) {
	...
	Engine._debug = true;
	Engine.initChannel = (options) => { return _yate.initChannel(options) };
	/**
	...
	/**
	 * @namespace Channel
	 * @description Channel object is only available in direct script execution mode from callto context with '{channel: true}' option.
	 * @example
	 * regexroute.conf:
 ...
 * @param {boolean} options.channel Direct run the script in channel mode ^NNN=extmodule/nodata/node.sh example.js
 * @see Yate#toChannel
 * @example
 * const {Yate, YateMessage} = require("next-yate");
...
class Yate extends EventEmitter {
	...
	/**
	 * Alternative initialization of script running from route context
	 * @method
	 * @async
	...
	toChannel() {
		 ...
		 * @method Channel.init
		 * @async
		 * @param {function} callback - runs just after initialization
