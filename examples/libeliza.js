...
module.exports = { Eliza }; // pack it to module
let _backup = Math.random; // replace original random function with Yate specific
Math.random = (min, max) => Math.floor(_backup() * (max - min) + min);
...
Eliza.replaceStr = function(strng, substr1, substr2, type) {
	...
	if (pntr < 0) return strng;
	return (
		strng.substr(0, pntr) +
		substr2 +
		Eliza.replaceStr(
			strng.substr(pntr + substr1.length),
			substr1,
			substr2,
			type
		)
...
Eliza.unpadString = function(strng) {
	for (;;) {
		...
		if (aString == strng) break;
		strng = aString;
	}
	if (strng.charAt(0) == " ") aString = aString.substr(1);
...
Eliza.findkey = function(wString) {
	for (var i = 0; i < Eliza.rules.length; i++) {
		...
		if (wString.indexOf(r) >= 0) return i;
	}
	return -1;
};
...
Eliza.prototype.listen = function(User) {
	...
	if (sInput == "") return "I can't help, if you will not chat with me!";

	var wInput = Eliza.padString(sInput.toUpperCase()); // Work copy
	var foundkey = -1;
	if (wInput != this.wPrevious)
		// check if user repeats himself
		foundkey = Eliza.findkey(wInput);
	else foundkey = Eliza.findkey(" **REPEAT** ");
	if (foundkey < 0) {
		if (!this.greet) {
			this.greet = true;
		...
		if (
			...
			if (sInput.length < 15) return "Tell me more...";
